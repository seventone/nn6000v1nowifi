...
# ---------- 每次开机扫尾：清理 S20 netifd 启动后新生成的空壳匿名 device section ----------
# 时序说明（backup-v4.3 新增，子坑 9-c）：
#   preinit 阶段：先跑 /etc/uci-defaults/99-nn6000v1nowifi → 此时 99- 的原子重建
#                确实能把 uci-defaults 执行之前遗留的所有匿名 device 扫干净；
#   S20 network 阶段（99- 跑完之后）：netifd 读取 /etc/board.json 生成网络配置，
#                内部会多次 `uci add network device` 但不写 name/type，留下 4 个
#                "裸空壳匿名 section"（实测任何遍历判断逻辑都会漏）。
#   所以必须再加一个 START=21 的 init 脚本，在 S20 network 之后、S40 firewall
#   之前再做一次"暴力清空 + 原子重建"，把 netifd 新加的 4 个空壳最终扫干净。
#   结果：每次开机结束后，`uci show network | grep -c @device` 必然 = 1。
INITD_DIR="./package/base-files/files/etc/init.d"
RCD_DIR="./package/base-files/files/etc/rc.d"
CLEANUP_SVC="nn6000-cleanup"
mkdir -p "$INITD_DIR" "$RCD_DIR"
cat > "$INITD_DIR/$CLEANUP_SVC" <<'INITD_EOF'
#!/bin/sh /etc/rc.common
# NN6000 V1 no wifi 旁路由：每次开机原子重建匿名 device section（仅保留1个br-lan）
# START=21 → 晚于 S20 network（netifd 启动），早于 S40 firewall
START=21
STOP=0
USE_PROCD=0

start() {
	# Step 1：命名 device section 清光（理论上首次启动 99- 已经清过，这里再兜底一次）
	uci delete network.lan1 2>/dev/null || true
	uci delete network.lan2 2>/dev/null || true
	uci delete network.lan3 2>/dev/null || true
	uci delete network.br_lan 2>/dev/null || true

	# Step 2：暴力清空所有匿名 device section → 永远只删 0 号，后面的自动前移
	while uci -q show "network.@device[0]" > /dev/null 2>&1; do
		uci -q delete "network.@device[0]" 2>/dev/null || true
	done

	# Step 3：明确新建 1 个匿名 device（必然是 @device[0]）
	uci add network device > /dev/null
	...
	uci set "network.@device[0].name=br-lan"
	# ⚠ MAC 只用一层双引号，不要再嵌套单引号（子坑 9-b 教训）
	uci set "network.@device[0].macaddr=f8:5e:3c:4c:50:7a"
	# ports：先清后加
	for _p in lan1 lan2 lan3; do
		uci -q del_list "network.@device[0].ports=$_p" 2>/dev/null
		uci add_list "network.@device[0].ports=$_p" 2>/dev/null || true
	...
	# Step 5：必须 commit —— 晚于 netifd，晚于 firewall，早于 LuCI 页面读取
	uci commit network

	# Step 6（可选）：netifd 重新读一次 device 状态，不重启网口
	# 注意：不需要 service network restart（会掉网），也不需要 ifdown/ifup lan；
	# 空壳 section 本身就不会被 netifd 创建成接口，删掉后也不会影响已启动的网桥。
	# 但执行一次 ubus call network reload 可以让 netifd 重新读 UCI 状态，
	# 保证 /var/state/network 与 /etc/config/network 同步（LuCI 展示会更一致）
...
INITD_EOF
chmod +x "$INITD_DIR/$CLEANUP_SVC"
# enable：rcS 启动时 S21 顺序自动 start
ln -sf "../init.d/$CLEANUP_SVC" "$RCD_DIR/S21$CLEANUP_SVC"
